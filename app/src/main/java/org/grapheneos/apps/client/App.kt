...
class App : Application() {
    ...
    private val tasksInfo = MutableLiveData<List<TaskInfo>>()
    ...
    fun getTasksInfo() : LiveData<List<TaskInfo>> = tasksInfo

    private fun MutableLiveData<List<TaskInfo>>.replaceOrPut(updatedTask : TaskInfo) :List<TaskInfo>{
        var found = false
        val result : MutableList<TaskInfo> = mutableListOf()
        value?.let {
            result.addAll(it)
        }
    ...
    private fun MutableLiveData<List<TaskInfo>>.updateStatus(infos : List<TaskInfo>){
        postValue(infos)
        if(!isServiceRunning){
    ...
    fun handleOnClick(
        ...
        when (status) {
            ...
            is InstallStatus.Installable -> {
                downloadPackages(variant, true) { error ->
                    callback.invoke(error.genericMsg)
                }
            }
            is InstallStatus.Installed -> {
                callback.invoke("$pkgName is being uninstalled")
                PackageManagerHelper().uninstall(this, pkgName)
            }
            ...
            is InstallStatus.Updated -> {
                callback.invoke("$pkgName is already up to date")
                PackageManagerHelper().uninstall(this, pkgName)
            }
            is InstallStatus.Failed -> {
                println("$pkgName installation failed :( ")
                callback.invoke(status.errorMsg)
            }
            is InstallStatus.Updatable -> {
                callback.invoke("$pkgName is being updated")
                CoroutineScope(scopeApkDownload).launch(Dispatchers.IO) {
                    downloadPackages(variant, true) { error ->
                        callback.invoke(error.genericMsg)
                    }
